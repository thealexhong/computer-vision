...
if isunix % for *nix machines (Linux, OSX, etc)
...
for useImageSet = 5 % 1:6  or just choose one image set, e.g. 3
  switch useImageSet
   case 1
    name = 'gray';
   case 2
  ...
  imDataRGB = zeros(numPixels, 3, nDir);
  
  % Read RGB components for each image
  for i = 1:nDir
      fname = [imageDir, name, '.', num2str(i-1), '.ppm'];
      RGBim = ppmRead(fname);
      imDataRGB(:,:,i) = reshape(RGBim(:),[numPixels, 3]);
  end
  
  % Mask each colour channel (RGB) of image
  imDataCropRGB = imDataRGB(mask, :, 1:nDirChrome);
  
  % Compute albedo for each colour channel
  for i = 1:3
    albedo(mask,i) = dot(squeeze(imDataCropRGB(:,i,:))', (nCrop * L)') ./ dot((nCrop * L)', (nCrop * L)');
  end
